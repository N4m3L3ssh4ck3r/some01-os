...
from configparser import ConfigParser ## for reading the config file in /etc/some01

class Repo():
    def __init__(self) -> None:
        self.repo_file = "/etc/some01/apps_repo/some01_apps.json"
        ...
        self.config_path="/etc/some01/summon.conf"
    ...
    ## Filter the some01_apps.json file for a single application:
    ...
    def display_app_info(self,repo_json,category,app):
        ...
        print(f" {self.style.subpipe}{self.style.PPIN}Summon Version{self.style.PPUR}: {self.style.CMNT}{repo_json['apps_list'][category][app]['some01_version']}{self.style.RST}")
    ...
    ## Update the Summon repository in /etc/some01/:
    def upgrade_summon(self):
        ...
        if self.update_check():
            ...
            if apps.git_pull(self.summon_path):
                ...
                shell.run_cmd(["rm","-rf","/etc/some01/apps_repo/*"]) ## Remove the old repository
                shell.run_cmd(["cp",self.summon_path+"/files/apps_repo/some01_apps.json","/etc/some01/apps_repo/"])
    ...
    def update_check(self):
        ...
        if xfce4_panel_icon_file!="":
            ...
            for index,line in enumerate(lines):
                if re.search("^Exec",line):
                    lines[index]=line.replace("/opt/some01",self.summon_path) ## Overwrite it.
        ...
        if os.path.exists(self.repo_file): ## We need a version first
            ...
            if local_version == current_version:
                ...
                shell.run_cmd(["notify-send","Summon",f"Summon Updates Available ({current_version})","--icon=/usr/share/some01/images/icons/summon.png"])
                xfce4_panel_icon_file = self.get_summon_icon_file() ## Just get the file name
                if xfce4_panel_icon_file!="":
                    xfce4_panel_icon_file=xfce4_panel_icon_file
    ...
    def check_config_file(self):
        if not os.path.exists("/etc/some01/summon.conf"):
            ...
            with open("/etc/some01/summon.conf","w") as config_file:
                config.write(config_file)

    def update_autostart_script(self,user):
        home_dir=f"/home/{user}/.config/autostart/"
        script=str(subprocess.run(["egrep","-iElr","opt.some01",home_dir],stdout=subprocess.PIPE).stdout.decode().strip())
        import re
        if script != "":
            ...
            for index,line in enumerate(script_lines):
                if re.search(r'^Exec',line):
                    script_lines[index]=line.replace("/opt/some01",self.summon_path)
            #print(script_lines)
    ...
    def restore_repo(self):
        ...
        if os.path.exists("files/apps_repo/some01_apps.json"):
            shell.run_cmd(["cp","files/apps_repo/some01_apps.json","/etc/some01/apps_repo/some01_apps.json"])
            self.update_check()
        else:
            print(f"{self.style.fail} Could not open local repo file for reading: files/apps_repo/some01_apps.json !")
            exit()
