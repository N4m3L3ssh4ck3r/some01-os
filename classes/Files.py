...
class Files:
    def __init__(self):
        self.some01_download_uri = "https://some01.tz/download/packages/1.0/"
        self.style = Style()
    ...
    def download_base_files(self):
        download_files = [
            {"uri":self.some01_download_uri+"some01.zip","path":"/usr/share/some01.zip","zip":True},
            {"uri":self.some01_download_uri+"ubuntu-font.zip","path":"/usr/share/fonts/truetype/ubuntu-font.zip","zip":True},
            {"uri":self.some01_download_uri+"emojione-android.ttf","path":"/usr/share/fonts/truetype/emojione-android.ttf","zip":False}
        ]
        ...
        for file in download_files:
            ...
            if not os.path.exists(file['path']):
                self.download_file(file['uri'],file['path'],False) ## Download some01.zip
    ...
    def build_redteam(self):
        with Status(f"{self.style.sub}{self.style.CMNT}Building {self.style.RST}{self.style.RED}/redteam{self.style.CMNT} and {self.style.RST}{self.style.RED}~/Pentest{self.style.CMNT} directory trees{self.style.RST}") as status:
            directories = ["/redteam","/redteam/windows-domains","/redteam/web",
                ...
                "/redteam/apple","/redteam/cloud","/etc/some01","/redteam/evasion","/redteam/c2" ]
