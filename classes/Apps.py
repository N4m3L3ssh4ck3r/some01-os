...
class Apps:
    def __init__(self):
        ...
        self.shell=Shell()
        self.files=Files()
        self.some01_app_repo="/etc/some01/apps_repo/some01_apps.json"
    ...
    def redteam_apps(self,all,force):
        ...
        if os.path.isfile(self.some01_app_repo):
            with open(self.some01_app_repo, "r") as config:
                ...
                for category in config_json['apps_list']:
                    for app in config_json['apps_list'][category]:
                        ...
                        if self.install_redteam_app(app,force,False): # Will return True or False
                            ## Update some01_apps.json and set the value to "True" for "installed":
            ...
            with open(self.some01_app_repo,"w") as update_json: ## The "indent=(n)" option here keeps the file linted/formatted/pretty nicely.
            ...
            print(f"{self.style.fail} Could not read {self.some01_app_repo} file.")
            exit()
                                ...
                                if not force:
                                    if app_module.check_install(): ## Already installed.
                                        ...
                                        if config_json['apps_list'][category][repo_app]['installed'] != "True":
                                            self.update_repo_app_installed(app) ## update the repo file in /etc/some01
                                        return True
