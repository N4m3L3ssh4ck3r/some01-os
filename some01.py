            ...
            if len(sys.argv)>2:
                if sys.argv[2]=="some01":
                    ...
                    if "-u" in sys.argv: ## Adding support for a user now:
                        ...
                        user = None
                    app_module = importlib.import_module("files.install_modules.some01")
...
def check_repo():
    apps = Apps()
    apps.git_behind(os.getcwd(),"some01")

## Show some01 repo Version
def some01_version():
    repo=Repo()
    repo.update_check()
    return
...
def redteam_apps(all,force):
    ...
    print(f"{style.sing} {style.BOLD}{style.PINK}some01ing the {style.CMNT}{style.RED}some01{style.PINK} for Apps ... {style.RST}")
    print(f"{style.sing} {style.BOLD}{style.PINK}Heading to the {style.RED}Crossroads{style.PINK} ... {style.RST}")
